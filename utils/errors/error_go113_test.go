...
func TestIs(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run("", func(t *testing.T) {
			...
			if tc.err != nil {
				// 排除nil的情况
				if got := Is(Errorf(tc.err, ""), tc.target); got != tc.match {
					t.Errorf("Is(%v, %v) = %v, want %v", tc.err, tc.target, got, tc.match)
				}
			}
			if tc.err != nil {
				// 排除nil的情况
				if got := Is(Errorf(Errorf(tc.err, ""), ""), tc.target); got != tc.match {
					t.Errorf("Is(%v, %v) = %v, want %v", tc.err, tc.target, got, tc.match)
				}
			}
		})
...
func TestAs(t *testing.T) {
	...
	for i, tc := range testCases {
		...
		t.Run(name, func(t *testing.T) {
			match := errors.As(Errorf(tc.err, ""), tc.target)
			if match != tc.match {
				t.Fatalf("match: got %v; want %v", match, tc.match)
			}
			if !match {
		...
		t.Run(name, func(t *testing.T) {
			match := errors.As(Errorf(Errorf(tc.err, ""), ""), tc.target)
			if match != tc.match {
				t.Fatalf("match: got %v; want %v", match, tc.match)
			}
			if !match {
...
func TestUnwrap(t *testing.T) {
	...
	for _, tc := range testCases {
		...
		if got := errors.Unwrap(Errorf(tc.err, "")); got != tc.want {
			t.Errorf("Unwrap(%v) = %v, want %v", tc.err, got, tc.want)
		}
		if got := errors.Unwrap(Errorf(Errorf(tc.err, ""), "")); got != tc.want {
			t.Errorf("Unwrap(%v) = %v, want %v", tc.err, got, tc.want)
		}
	}
}
...
func ExampleAs() {
	if _, err := os.Open("non-existing"); err != nil {
		...
		if errors.As(Errorf(err, ""), &pathError) {
